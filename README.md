...
curl -fsSL https://raw.githubusercontent.com/batorfi/pipeline-template/main/install.sh \
  | bash -s -- --template-version v0.1.24 --target ./my-project
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/batorfi/pipeline-template/main/install.sh \
  | bash -s -- --template-version v0.1.24 --target ./my-existing-repo
```
...
If `specify` from step 1 is on `PATH`, either form runs `specify init . --integration claude` inside the target automatically. (Or, from an existing local clone of this repo: `scaffold/scaffold.sh --template-version v0.1.24 --target <path>`.)
...
The cmux-workspace step uses [`docs/manual-cmux-workspace-setup.md`](docs/manual-cmux-workspace-setup.md) — run the `cmux` CLI commands yourself, including a naming convention (`<project>-main`, `<project>-design`, `<project>-implementation`) so the cmux sidebar reads clearly at a glance. The dashboard step uses [`scaffold/prompts/run-dashboard-in-pane.md`](scaffold/prompts/run-dashboard-in-pane.md) (paste its prompt into a Claude Code session running inside cmux — copied into your scaffolded project as `docs/prompts/run-dashboard-in-pane.md`; see "How to run a `docs/prompts/*.md` file" in [`docs/scaffolding-guide.md`](docs/scaffolding-guide.md)) or the manual commands in [`docs/running-the-dashboard.md`](docs/running-the-dashboard.md).
...
| Stand up the 3 cmux workspaces with a real name→ID mapping | [`docs/manual-cmux-workspace-setup.md`](docs/manual-cmux-workspace-setup.md) |
...
- ✅ [`docs/manual-cmux-workspace-setup.md`](docs/manual-cmux-workspace-setup.md) — **confirmed against a real cmux instance**: both `cmux new-workspace` and `cmux rename-workspace` print the workspace's ID directly in their own output (`OK workspace:<n>`), so the before/after `list-workspaces` diff an earlier draft assumed as a fallback isn't needed; the JSON key for a workspace's ID is `ref` (`workspace_ref` one level up in `current-workspace --json`). Real run also caught and fixed a doc bug: an earlier draft's copy-pasteable example used a literal `<design-id>` placeholder in an unquoted bash command, which the shell parsed as input redirection (`workspace:16: no such file or directory`) instead of a value to substitute — steps now build IDs into shell variables instead of literal angle-bracket placeholders.
