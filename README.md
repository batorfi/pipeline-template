...
curl -fsSL https://raw.githubusercontent.com/batorfi/pipeline-template/main/install.sh \
  | bash -s -- --template-version v0.1.18 --target ./my-project
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/batorfi/pipeline-template/main/install.sh \
  | bash -s -- --template-version v0.1.18 --target ./my-existing-repo
```
...
If `specify` from step 1 is on `PATH`, either form runs `specify init . --integration claude` inside the target automatically. (Or, from an existing local clone of this repo: `scaffold/scaffold.sh --template-version v0.1.18 --target <path>`.)
...
- ✅ `dashboard/` — backend (5 endpoints, FastAPI) and frontend (4-zone vanilla JS), integration-tested, confirmed rendering in cmux's actual embedded browser. **Real scaffolded-project bug found and fixed**: the backend imports `factory-log/validator.py` from a fixed project-root-relative path at runtime, but scaffolding never actually copied `factory-log/` into the project — every scaffolded project's dashboard failed to start until this was fixed. `do_copy_steps` and `--sync` now copy `factory-log/validator.py` and `factory-log/SCHEMA.md`; confirmed against a real scaffolded project, backend now starts and both `/log` and `/` respond correctly.
