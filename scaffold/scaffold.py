...
def ensure_gitignore(clone_dir: Path, target: Path) -> None:
    # A scaffolded project previously got no .gitignore at all, from either
    # a fresh scaffold or --sync -- so .specify/cmux-workspaces.json (opaque
    # IDs meaningful only to one person's local cmux socket) and
    # dashboard/config.json (generated, local) would get committed by
    # default on the first `git add -A`. Confirmed as a real, already-shipped
    # bug: found already committed in a real scaffolded project three
    # features in. Appends the missing lines to an existing .gitignore
    # (never overwrites a project's own rules) or creates one if none
    # exists. Idempotent -- safe to call on every scaffold/sync.
    additions_path = clone_dir / "scaffold" / "gitignore-additions.txt"
    additions_text = additions_path.read_text(encoding="utf-8")
    addition_pattern_lines = [
        line for line in additions_text.splitlines() if line.strip() and not line.strip().startswith("#")
    ]

    gitignore_path = target / ".gitignore"
    if not gitignore_path.exists():
        gitignore_path.write_text(additions_text, encoding="utf-8")
        return

    existing_text = gitignore_path.read_text(encoding="utf-8")
    existing_set = set(existing_text.splitlines())

    # Only the patterns actually missing get appended — never the whole
    # block, or every re-run against an existing .gitignore would duplicate
    # whatever was already there.
    missing_lines = [line for line in addition_pattern_lines if line not in existing_set]
    ...
    with open(gitignore_path, "a", encoding="utf-8") as fh:
        if not existing_text.endswith("\n"):
            fh.write("\n")
        fh.write("\n# Added by pipeline-template's scaffold.sh — see scaffold/gitignore-additions.txt\n")
        fh.write("\n".join(missing_lines) + "\n")
...
def scaffold(template_version: str, target: str) -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        try:
            ...
            ensure_gitignore(clone_dir, target_path)
