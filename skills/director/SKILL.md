...
4. `.specify/cmux-workspaces.json` exists and contains `main`, `design`, and `implementation` workspace IDs — cmux's CLI has no workspace-naming concept, only opaque IDs, so this file is how you actually address "the design workspace" with a real `--workspace <id>` flag instead of a name cmux doesn't understand. If this file is missing, stop and ask the human to run the `docs/scaffolding-guide.md` step 2 setup prompt (`scaffold/prompts/setup-cmux-workspaces.md`) — do not guess at workspace IDs or create workspaces yourself as a workaround.
